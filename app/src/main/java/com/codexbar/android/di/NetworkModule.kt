...
@Qualifier
@Retention(AnnotationRetention.BINARY)
annotation class DeepSeekClient

@Qualifier
@Retention(AnnotationRetention.BINARY)
annotation class MiMoClient
...
object NetworkModule {
    ...
    @Singleton
    @DeepSeekClient
    fun provideDeepSeekOkHttpClient(): OkHttpClient = baseOkHttpBuilder().build()

    @Provides
    @Singleton
    fun provideDeepSeekApiService(
        @DeepSeekClient client: OkHttpClient,
        json: Json
    ): DeepSeekApiService {
        return Retrofit.Builder()
    ...
    @Singleton
    @MiMoClient
    fun provideMiMoOkHttpClient(): OkHttpClient = baseOkHttpBuilder().build()

    @Provides
    @Singleton
    fun provideMiMoApiService(
        @MiMoClient client: OkHttpClient,
        json: Json
    ): MiMoApiService {
        return Retrofit.Builder()
            .baseUrl(AiService.MIMO.baseUrl)
            .client(client)
            .addConverterFactory(json.asConverterFactory("application/json".toMediaType()))
            .build()
