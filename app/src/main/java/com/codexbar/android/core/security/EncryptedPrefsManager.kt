...
class EncryptedPrefsManager @Inject constructor(
    ...
    fun saveCredential(service: AiService, credential: Credential) {
        ...
        when (credential) {
            is Credential.ClaudeCredential -> {
                credential.expiresAt?.let { editor.putLong("${prefix}_expires_at", it.epochSecond) }
                credential.scopes?.let { editor.putString("${prefix}_scopes", it) }
                credential.rateLimitTier?.let { editor.putString("${prefix}_rate_limit_tier", it) }
            }
            is Credential.CodexCredential -> {
                if (!credential.accountId.isNullOrBlank()) editor.putString("${prefix}_account_id", credential.accountId)
                else editor.remove("${prefix}_account_id")
                if (!credential.manualResponse.isNullOrBlank()) editor.putString("${prefix}_manual_response", credential.manualResponse)
                else editor.remove("${prefix}_manual_response")
            }
            is Credential.CodexFeelolCredential -> {
                if (!credential.manualResponse.isNullOrBlank()) editor.putString("${prefix}_manual_response", credential.manualResponse)
                else editor.remove("${prefix}_manual_response")
            }
            ...
            is Credential.ChatGPTPlusCredential -> {
                ...
                if (!credential.manualSessionResponse.isNullOrBlank()) {
                    editor.putString("${prefix}_manual_session_response", credential.manualSessionResponse)
                } else {
                    editor.remove("${prefix}_manual_session_response")
                }
            }
    ...
    fun loadCredential(service: AiService): Credential? {
        ...
        return when (service) {
            AiService.CLAUDE -> {
                ...
                Credential.ClaudeCredential(accessToken, refreshToken, expiresAt, scopes, rateLimitTier)
            }
            AiService.CODEX -> {
                ...
                Credential.CodexCredential(accessToken, refreshToken, accountId, manualResponse)
            }
            AiService.CODEX_FEELOL -> {
                val manualResponse = prefs.getString("${prefix}_manual_response", null)
                if (accessToken.isBlank() && manualResponse.isNullOrBlank()) return null
                Credential.CodexFeelolCredential(accessToken = accessToken, manualResponse = manualResponse)
            }
            AiService.GEMINI -> {
                if (accessToken.isBlank()) return null
                val refreshToken = prefs.getString("${prefix}_refresh_token", null) ?: return null
                val expiresAtMs = prefs.getLong("${prefix}_expires_at_ms", -1L).takeIf { it > 0 } ?: return null
                ...
                Credential.GeminiCredential(accessToken, refreshToken, expiresAtMs, clientId, clientSecret)
            }
            AiService.DEEPSEEK -> {
            ...
            AiService.CHATGPT_PLUS -> {
                ...
                Credential.ChatGPTPlusCredential(accessToken = accessToken, planName = planName, renewalDate = renewalDate, billingPeriod = billingPeriod, notes = notes, manualSessionResponse = manualSessionResponse)
            }
            AiService.MIMO -> {
                ...
                Credential.MiMoCredential(accessToken = accessToken, backendUrl = backendUrl, backendToken = backendToken, directCookie = directCookie, useBackendMode = useBackendMode)
            }
        }
    }
    ...
    fun isProviderEnabled(service: AiService): Boolean = getEnabledServices().contains(service)
    ...
    private fun setProviderEnabled(service: AiService, enabled: Boolean, editor: SharedPreferences.Editor) {
        val names = getEnabledServices().map { it.name }.toMutableSet()
        if (enabled) names.add(service.name) else names.remove(service.name)
        editor.putStringSet(KEY_ENABLED_SERVICES, names)
    }

    fun getDashboardTheme(): DashboardThemeStyle {
        val name = prefs.getString(KEY_DASHBOARD_THEME, DashboardThemeStyle.SYSTEM.name) ?: DashboardThemeStyle.SYSTEM.name
    ...
    fun getRefreshInterval(): Long = prefs.getLong("refresh_interval_minutes", 30L)
